  ...
  setup(props, { slots, emit, expose }) {
    ...
    const parsedColumnCount = computed(() => {
      return days.value.length +
        ...
        days.value.length ===
        1 && parseInt(props.columnCount, 10) > 0
        ? parseInt(props.columnCount, 10)
        : 0
    })
      ...
      if (days.value.length === 1 && parseInt(props.columnCount, 10) > 0) {
        return [
          isLeftColumnOptionsValid.value === true &&
            props.leftColumnOptions.map((column, index) => __renderHeadColumn(column, index)),
          ...
          isRightColumnOptionsValid.value === true &&
            props.rightColumnOptions.map((column, index) => __renderHeadColumn(column, index)),
        ]
      } else {
        return [
          isLeftColumnOptionsValid.value === true &&
    ...
    function __renderDays() {
      if (days.value.length === 1 && parseInt(props.columnCount, 10) > 0) {
        return [
          isLeftColumnOptionsValid.value === true &&
            props.leftColumnOptions.map((column, index) => __renderColumn(column, index)),
          Array.apply(null, new Array(parseInt(props.columnCount, 10)))
            .map((_, i) => i + parseInt(props.columnIndexStart, 10))
            .map((i) => __renderDay(days.value[0], 0, i)),
          isRightColumnOptionsValid.value === true &&
            props.rightColumnOptions.map((column, index) => __renderColumn(column, index)),
        ]
      } else {
        return [
          isLeftColumnOptionsValid.value === true &&
    ...
    function __renderDay(day, dayIndex, columnIndex) {
      ...
      style.height =
        parseInt(props.dayHeight, 10) > 0 ? convertToUnit(parseInt(props.dayHeight, 10)) : 'auto'
